    ...
    asyncHandler(async (request, response) => {
      ...
      const integration = await upsertWhatsAppIntegration(
        options.supabaseAdminClient,
        {
          ...
          businessPhoneNumberId: parsed.data.phoneNumberId,
          businessAccountId: parsed.data.businessAccountId || null,
          whatsappBusinessAccountId: parsed.data.businessAccountId || null,
          phoneNumber: parsed.data.displayPhoneNumber || null,
          displayPhoneNumber: parsed.data.displayPhoneNumber || null,
          accessToken,
          verifyToken,
          webhookSecret: parsed.data.webhookSecret || null,
          status: parsed.data.isActive ? "connected" : "pending",
          webhookSubscribed: existingIntegration?.webhook_subscribed ?? false,
          onboardingPayload: existingIntegration?.onboarding_payload ?? {},
